...
SOURCE_DIR = ROOT_DIR.joinpath("zotero_fastapi_server")
...
PYTHON_VERSION = 3.9
CI_PROJECT_NAME = "zotero-fastapi-server"
CI_REGISTRY_IMAGE = "registry.gitlab.com/https://github.com/LARA-literature/zotero-fastapi-server"
...
def docker_test(_c, python_version=PYTHON_VERSION):
    ...
    """
    volume_mount = (
        f"--volume {BIN_DIR}:/zotero_fastapi_server/bin/ --volume {ROOT_DIR}:/zotero_fastapi_server:rw"
    )
    registry_path = _get_registry_path_str(python_version)
    pytest_arg = f"pytest -v --cov-report xml:/zotero_fastapi_server/bin/coverage.xml {TEST_DIR}"
    Path(BIN_DIR).mkdir(parents=True, exist_ok=True)
