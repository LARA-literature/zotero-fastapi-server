CODE_PATHS := zotero_fastapi_server
...
CI_REGISTRY_IMAGE			?= ${CI_REGISTRY}/https://github.com/LARA-literature/zotero-fastapi-server
...
.build_${PYTHON_VERSION}: Dockerfile zotero_fastapi_server clean__py
	${_CMD_DOCKER_BUILD} \
...
test:	## run pytest on mounted-in tests (change tests, no rebuild!)
	...
	docker run \
			--platform linux/amd64 \
			--volume $(shell pwd)/bin/:/zotero_fastapi_server/bin/ \
			--volume $(shell pwd):/zotero_fastapi_server:rw \
			${DOCKER_IMAGE_TAG} \
			pytest --cov-report xml:/zotero_fastapi_server/bin/coverage.xml tests/
